...
def _convert_indicator_to_dataframe(indicator: dict) -> Optional[pd.DataFrame]:
    ...
    if not records:
        return None
    records = pd.concat(records, 0).reset_index().rename(columns={'index': 'instrument'})
...
def single(
    ...
    if generate_report:
        ...
        result = records
    assert not np.isnan(records['ffr']).any()
...
def backtest(backtest_config: dict) -> pd.DataFrame:
    ...
    if generate_report:
        with (Path('outputs') / 'report.pkl').open('wb') as f:
            report = {}
            for r in res:
                report.update(r[1])
    ...
    res.to_csv(Path('outputs') / 'summary.csv')
    return res
